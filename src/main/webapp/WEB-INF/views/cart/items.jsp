		...
		<div class="clearfix container">
			...
			<div id="header-content">
				<nav id="main-nav">
					...
					<ul class="clearfix">
						<li><a href="${s:mvcUrl('CSC#items').build()}" rel="nofollow">Carrinho (${cart.quantity})</a></li>
		        ...
		        <c:forEach items="${cart.items}" var="item">
			      <tr>
			          ...
			          <td class="quantity-input-cell">
			                <input type="number" min="0" readonly="readonly" id="quantity" name="quantity" value="${cart.getQuantity(item)}"/>
			          </td>
			          <td class="numeric-cell">${cart.getTotal(item)}</td>
			          <td class="remove-item">
			            <form action="${s:mvcUrl('CSC#remove').arg(0, item.product.id).arg(1,item.priceType).build() }" method="post">
			      ...
			      <tfoot>
			        <tr>
			          <td colspan="3">
                        <form action="${s:mvcUrl('PC#pay').build()}" method="post">
